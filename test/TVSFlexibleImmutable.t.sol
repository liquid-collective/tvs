...
contract TVSFlexibleImmutableExt is TVSFlexibleImmutable, MockTarget {    
    constructor(address beneficiary, address owner, address withdrawalContractAddress, address consolidationContractAddress) TVSFlexibleImmutable(beneficiary, owner, withdrawalContractAddress, consolidationContractAddress) {}
}
...
contract TVSFlexibleImmutableTest is TVSImmutableTest{
    ...
    function test_ExecuteCallWhen_IsDelegateCallIsFalse() public {
        ...
        bytes memory data = getCallData(3);
        TVSFlexibleImmutable.Call memory callWithoutValue = TVSFlexibleImmutable.Call({
            to: address(target),
            value: 0,
            data: data,
            isDelegateCall: false
        ...
        TVSFlexibleImmutable.Call memory callWithValue = TVSFlexibleImmutable.Call({
            to: address(target),
            value: _value,
            data: data,
            isDelegateCall: false
    ...
    function test_ExecuteBatch_ShouldBeAtomic() public {
        ...
        calls[0] = TVSFlexibleImmutable.Call({
            to: address(target),
            value: 0,
            data: getCallData(42),
            isDelegateCall: false
        ...
        vm.expectRevert(); // Not concerned with the exact reason, we care only that one bad call should fail the whole batch
    ...
    function test_ExecuteBatch_TargetCalledNTimes(uint n) public {
