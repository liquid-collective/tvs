...
interface ISweepBeneficiary {
    
    /// @notice Allows a contract to receive ETH from TVS via the `sweepToContract` function.
    /// @dev This function MUST be implemented by the TVS beneficiary contract, in order to use the `sweepToContract` function of the TVS.
